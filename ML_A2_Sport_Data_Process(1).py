...
# TODO Step size needs to be determined.
step_size = 10    # 定义步长
features_list = []

feature_list_orientation = []
feature_list_accelerate = []
feature_list_accelerometer = []
...
for motion in motions:
    for sensor in sensors:
        ...
        if sensor == '/Orientation':
            ...
            feature_list_orientation = feature_extract(columns)
        elif sensor == '/Accelerometer':
            columns = ['x', 'y', 'z']
            feature_list_accelerometer = feature_extract(columns)
        else:
            columns = ['x','y','z']
            feature_list_accelerate = feature_extract(columns)
                
    
    
        
    # 将特征列表转换为DataFrame
    feature_df_a = pd.DataFrame(feature_list_accelerometer)

    feature_df_a.to_csv('feature_df_a.csv', sep='\t', index=False, encoding='utf-8')
        
    # 将特征列表转换为DataFrame
    feature_df_t = pd.DataFrame(feature_list_accelerate)

    feature_df_t.to_csv('feature_df_t.csv', sep='\t', index=False, encoding='utf-8')
    # 将特征列表转换为DataFrame
    feature_df_o = pd.DataFrame(feature_list_orientation)

    feature_df_o.to_csv('feature_df_o.csv', sep='\t', index=False, encoding='utf-8')
    # 将特征添加到列表中
    features_list = np.concatenate((feature_list_orientation, feature_list_accelerometer, feature_list_accelerate))
